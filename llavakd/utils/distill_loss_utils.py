...
def forward_kl(logits, teacher_logits, no_model_batch):
    ...
    mask = (no_model_batch["label"] != IGNORE_INDEX).int()
...
def skewed_reverse_kl_womask(logits, teacher_logits, labels, lam=0.1):
    ...
    # Create inf_mask to handle numerical instabilities
    inf_mask = torch.isinf(logits) | torch.isinf(teacher_logits)
    ...
    # Compute the distillation loss without masking
    distil_loss = -torch.mean(x, dim=0)
    
    return distil_loss

def reverse_kl_womask(logits, teacher_logits, labels):
    student_probs = F.softmax(logits, dim=-1, dtype=torch.float32)
    student_logprobs = F.log_softmax(logits, dim=-1, dtype=torch.float32)
    teacher_logprobs = F.log_softmax(teacher_logits, dim=-1, dtype=torch.float32)
    inf_mask = torch.isinf(teacher_logits) | torch.isinf(logits)
