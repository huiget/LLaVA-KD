...
# @register_vision_tower('opencip')      
class OpenCLIPVisionTower(nn.Module):
    def __init__(self, vision_tower, cfg, delay_load=False): # 这里应该要修改configuration_llava, 以增添新的属性
        super().__init__()

        self.is_loaded = False
        self.vision_tower_name = vision_tower
        self.vision_config = json.load(open(os.path.join(vision_tower,'open_clip_config.json'), 'r'))
        self.is_optimize = getattr(cfg, 'optimize_vision_tower_aux', False)
        self.is_droppath = getattr(cfg, 'drop_path', True)
